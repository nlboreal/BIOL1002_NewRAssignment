---
title: "BIOL 1002: Moose Ecology Study"
author: "Elizabeth Mack and Yolanda Wiersma"
date: "2025-04-09"
...
```{r, include=F, echo=F}
# additional information not include in on the course website
library(dplyr)

```

**Overview:** In these two assignments, we will be building on the R skills you learned in BIOL 1001. We will be focusing on manipulating and organizing data in tables, and learn a useful package called [dplyr](https://dplyr.tidyverse.org/) (pronounced "d-plier")
...
In addition, these exercises are based on *Week 3: Data in Tables* (PART I) and *Week 4: Grouping & Joining Data* (PART II) of the online course [Data Carpentry for Biologists](https://datacarpentry.org/semester-biology/schedule/) by Dr. Ethan White. If you are not sure how to execute a particular command, you can refer to the *Lecture* material on [Dr. White's website](https://datacarpentry.org/semester-biology/lectures/R-data/)</u> and/or his [instructional videos](https://datacarpentry.org/semester-biology/lectures/R-data/) that go with the relevant lessons. The example data in the lecture material and videos is different from the data you use in the exercise.
...
**TO HAND-IN** 
You are to hand in a `.R` file, formatted as shown in [Best Practices - Template](https://ahurford.github.io/quant-guide-all-courses/style.html#template) with questions and answers included as comments (`#`) with the lines of code to produce the output asked for included with each answer. 
------------------------------------------------------------------------
...
You must download these datasets before beginning this assignment. You can download them from Brightspace: You can also access these data from [here](https://github.com/jbaafi/quant-guide-data/tree/main/BIOL1002-data). Then, save all .csv files to an appropriate folder (E.g., BIOL1002_Coding).

## Part I: Moose Populations in Newfoundland

**Question 1**\
[Install](https://ahurford.github.io/quant-guide-all-courses/rintro.html#r-packages) the `dplyr` package using `install.packages()` function. You only need to do this once.\
Then, use the `library()` function to load `dplyr` so that its functions are available to use.

**Question 2** read.csv()\
Next [import](https://ahurford.github.io/quant-guide-all-courses/data-entry.html#loading-or-importing-data) the `MoosePopulation.csv` dataset. You may call your dataset `Moosedata` or give it a different name. If you use the programmatic way (i.e., command line), your command may look something like this:
...
**Question 4**\
Next, let's simplify the dataset to only include the columns of interest. Add a line of code where you use the [`select()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#selecting-a-column) function to select the following columns: `Ecoregion`, `Year`, `Area`, `Estimated_Moose_Pop`.
...
b.  Use the `max()` function to what is the highest 'Estimated_Moose_Pop' was recorded? Then write your answer as a comment `#`. Is this number for a particular ecoregion or for all of Newfoundland?

**Question 6**\
Different ecoregions cover different land areas, so comparing raw population numbers is not very helpful. Standardize your data by calculating moose density for each ecoregion. Using the [`mutate()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#adding-a-new-column) function, create a new column called `MooseDensity`. Remember density is equal to population divided by area.
...
**Question 7** plot()\
a. Now, let's visualize the data. Using the [`plot()`](https://ahurford.github.io/quant-guide-all-courses/graph.html#scatter-plots-with-plotxy) function, make a line graph to show the the changes in `MooseDensity` over `Year`. Full marks for including axis labels and figure title ( `xlab` , `ylab` , `main`).
...
a) Create a new dataset, where you use the [`filter()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#filtering-data) function to only include observations from the `Western_Forests` ecoregion.

```{r}
MooseDataWest <- filter(Moosedata, Ecoregion == "Western_Forests")
```
\
b) Use the `plot()` function to make a [line graph](https://ahurford.github.io/quant-guide-all-courses/graph.html#line-graphs) (`type = "l"`) showing how moose density has changed over time in Western_Forests region. Full marks for including axis labels and figure title ( `xlab` , `ylab` , `main`). *HINT:* Adapt the code given to you in Question 7 above. 
...
plot(MooseDataWest$Year, MooseDataWest$MooseDensity, 
     ...
     xlab = "year", 
     ylab = "Moose per sq km", 
     main = "Moose density over time in the Western Forest Ecoregion")
```
...
a. Using the **original, unfiltered dataset you created in Question 6 above**, use the [`filter()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#filtering-data) function to filter for the year `2020`, and save the dataset under a new name (e.g., MooseData_2020). HINT: Adapt the code given to you in Question 8 above.
...
b)  The research team considered moose densities above 2.0 moose/km² to be high. Using the dataset you just created, `filter()` the `MooseDensity` to only show ecoregions where moose density is greater than 2.0. Save the dataset under a new name (e.g., MooseData_2020_b). **HINT:** Adapt the code given to you in Question 8.

```{r, include=FALSE}
MooseDensity_2020_High <- filter(Moosedata_2020, MooseDensity > 2.0)
```

<!-- -->

c)  With the dataset you just created, use the [`arrange()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#sorting-the-data) function to sort the `MooseDensity` column in descending order. Template: `arrange(Dataset, desc(ColumnName))`

```{r, echo=FALSE}
arrange(MooseDensity_2020_High, desc(MooseDensity))
```
...
Repeat the steps from the previous question using a pipe at the end of each line. Using your inital dataset, `filter()` for the year `2020`, then `filter()` for `MooseDensity` above `2.0`, then `arrange()` in `desc()` order, then finally `print()` the output.
...
**Question 11**\
a. [Load](https://ahurford.github.io/quant-guide-all-courses/data-entry.html#loading-or-importing-data) the data file: `SaplingStudy.csv` and save your dataset to a name (e.g., Saplings).

```{r, include=FALSE}
SaplingData <- read.csv("SaplingStudy.csv")
```

b)  Then remove the `NA`s from the dataset using `na.omit()` function. Remember to save the cleaned version of your dataset to a new name (or override the old name).
...
**Question 12**\
a. How does moose browsing pressure vary across different ecoregions? With the help of [pipes](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#pipes) `%>%`, create a new database using the [`group_by()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#grouping) function to group the data by `Ecoregion`, and then use `summarize()` to calculate the `mean()` moose `BrowsingScore` for each site. Print the result using the `print()` function.

```{r, echo=FALSE}
SaplingData_Ecoregion <- SaplingData %>%
  group_by(Ecoregion) %>%
...
b)  Add a comment `#` saying which ecoregion has the most moose browsing. Which ecoregion has the lowest?

**Question 13**\

a) How does the average tree height vary across different ecoregions? With the help of [pipes](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#pipes) `%>%`, use the `group_by` function to group the data by `Ecoregion`, and then use `summarize()` to calculate the `mean()` tree `Height` for each group. Print the result using the `print()` function. *HINT:* Adapt the code from Question 12 above. 

```{r, include = F}
SaplingDataHeight <- SaplingData %>%
  group_by(Ecoregion) %>%
  summarize(AverageTreeHeight = mean(Height)) %>%
  print()
```

b)  The team considered average heights less than 20 cm to be severely browsed by moose. Add a short comment `#` to describe which ecoregions have average heights less than 20. Do this by looking at your data or using a filter(). Print the result using the `print()` function.

```{r, include=FALSE}
SevereSites <- SaplingDataHeight %>%
  filter(AverageTreeHeight < 20) %>%
  print()
```

**Question 14**\
a) How does the average browsing score vary across different tree sapling species? Use the `group_by` function to group the data by `Species`, and then use `summarize() to`calculate the `mean` tree `BrowsingScore` for each group. Print the result using the `print()` function. **HINT:** Adapt the code from Question 12. 

```{r, include = F}
BrowsedSpecies <- SaplingData %>%
  group_by(Species) %>%
  summarize(MeanBrowseScore = mean(BrowsingScore)) %>%
  print()
```

b)  Add a comment `#` saying which species has the highest browsing score, which species has the lowest?
...
`barplot(Dataset$YAxisColumn, 
        names.arg = Dataset$XAxisColumn,
        xlab = "X-axis name goes here",
        ylab = "Y-axis name goes here",
        main = "Figure title goes here",
        col = "forestgreen",  #pick any colour you want
        cex.names = 0.6)  # Reduces x-axis label size for readability`

```{r, include=FALSE}
barplot(BalsamFirImpacts$MeanBrowseScore, names.arg = BalsamFirImpacts$Ecoregion, xlab= "Ecoregion", ylab = "Browse Score", main = "Browse scores on balsam fir", col = "grey", cex.names = 0.6)
...
```{r, include=FALSE}
BlackSpruceImpacts <- SaplingData %>%
...
b)  Using the dataset you just created, make a bar graph the `barplot()` function to show the Average browsing score for Black Spruce across the ecoregions. Each bar should represent an ecoregion, and the height of the bar should reflect the average browsing intensity

```{r, include=FALSE}
barplot(BlackSpruceImpacts$MeanBrowseScore, names.arg = BlackSpruceImpacts$Ecoregion, xlab= "Ecoregion", ylab = "Browse Score", main = "Browse scores on balsam fir", col = "grey", cex.names = 0.6)
```

<!-- -->

c)  How does Black Spruce browsing compare to Balsam Fir browsing across ecoregions? Write 1-2 sentences as a comment `#` with your answer.
...
**Question 19**\
Were the same number of saplings counted for each species? Add a line of code where you use the `tally()` function to determine how many individual trees were counted in for each `Species`? *HINT:* Adapt the code from Question 18 above.
...
In this section, we will merge the `Moosedata`and the `Sapling` datasets, to explore how browsing intensity on different tree species correlates with moose density across ecoregions. You will practice using the left_join() function to combine these dataset
...
b) Use `left_join()` to join `MooseData2020` with the `Saplings` dataset, matching rows by the common `Ecoregion` column.
...
```{r BrowsingIntensitySpecies_plot, echo=FALSE, fig.align='center'}
library(ggplot2)
...
**Question 25** left_join\

a) Now let's join this dataset with `Moosedata` filtered for just the year 2020 (from Part One, question 9). Try to use `left_join()` to join them. *HINT:* follow the template of the code you used above in Pquestion 21. Why does this not work?

b)  Use the `rename_with()` function, to change the column name of one of the datasets to match the other, so that they can be joined. Here is some template code that you can adapt to make the necessary change. 
...
c)  Now join the datasets into a new dataset using `left_join()`. *HINT:* follow the template of the code you used above in question 21. 
...
plot(MooseMerged$MooseDensity, MooseMerged$Collisions_2020, xlab = "Moose Density", ylab = "Collisions")
```
