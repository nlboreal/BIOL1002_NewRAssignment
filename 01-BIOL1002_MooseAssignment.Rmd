...
**It is very important to name your data and objects as instructed in the following questions. Watch out for typos.**  
...
**Question 2** read.csv()\
Next [import](https://ahurford.github.io/quant-guide-all-courses/data-entry.html#loading-or-importing-data) the `MoosePopulation.csv` dataset. Name this dataset `moosedata`. If you use the programmatic way (i.e., command line), your command may look something like this:
...
a) Create a new dataset, where you use the [`filter()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#filtering-data) function to only include observations from the `Western_Forests` ecoregion. Save the result as `moose_west`.
...
a. Using the **original, unfiltered dataset you created in Question 6 above**, use the [`filter()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#filtering-data) function to filter for the year `2020`, and save the dataset as `moose_2020`. **HINT:** Adapt the code given to you in Question 8 above.
...
b)  The research team considered moose densities above 2.0 moose/km² to be high. Using the dataset you just created, `filter()` the `MooseDensity` to only show ecoregions where moose density is greater than 2.0. Save the dataset as `moose_2020_high`. **HINT:** Adapt the code given to you in Question 8.
...
c)  With the dataset you just created, use the [`arrange()`](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#sorting-the-data) function to sort the `MooseDensity` column in descending order. Save the result as `moose_2020_high_byD`. **HINT:** You can follow this template: 
...
**Question 11**\
a. [Load](https://ahurford.github.io/quant-guide-all-courses/data-entry.html#loading-or-importing-data) the data file: `SaplingStudy.csv` and save your dataset as `saplings`.

```{r, include=FALSE}
saplings <- read.csv("SaplingStudy.csv")
```

b)  Then remove the `NA`s from the dataset using `na.omit()` function. Remember to save the cleaned version of your dataset as `sap_clean`.
...
b)  Which ecoregions have the highest and lowest amount of moose browsing? Rearrange your dataset in order of decreasing average browsing score `AverageBrowsing`. Save the result as `avg_browse_reg`. Add a short comment `#` indicating which regions had the highest and lowest average browsing scores.

```{r, include = FALSE}
avg_browse <- arrange(sap_reg_browse, desc(AverageBrowsing))
```

**Question 13**\

a) How does the average tree height vary across different ecoregions? With the help of [pipes](https://ahurford.github.io/quant-guide-all-courses/handling-data.html#pipes) `%>%`, use the `group_by` function to group the data by `Ecoregion`, and then use `summarize()` to calculate the `mean()` tree `Height` for each group. Print the result using the `print()` function. Save the result as `sap_reg_height`. **HINT:** Adapt the code from Question 12 above. 
...
```{r, include=FALSE}
barplot(fir_reg_browse$MeanBrowseScore, names.arg = fir_reg_browse$Ecoregion, xlab= "Ecoregion", ylab = "Browse Score", main = "Browse scores on balsam fir", col = "grey", cex.names = 0.6)
...
a)  With the help of pipes, use the `filter()` function to filter the `Species` column for only `Black_Spruce`, then use `group_by()` function to sort by `Ecoregion` , and then determine `mean()` moose `BrowsingScore`. Save the resulting summary table as `spruce_reg_browse`.
...
```{r, include=FALSE}
barplot(spruce_reg_browse$MeanBrowseScore, names.arg = spruce_reg_browse$Ecoregion, xlab= "Ecoregion", ylab = "Browse Score", main = "Browse scores on balsam fir", col = "grey", cex.names = 0.6)
```
...
a)  To correct this and join our datasets we can use the `rename_with()` function. Here is some template code that you can adapt to make the necessary change. Rename the column holding site information in the `moose_coll` dataset and save the renamed result as `moose_coll2`.
...
b)  Now join the datasets into a new dataset using `left_join()`. Save the joined dataset as `coll_merge`. **HINT:** follow the template of the code you used above in question 21. 
...
**Question 30**\
Ensure that your R script can run fully from start to finish without errors. From within RStudio, find and click the "Source" button at the top-right of the window holding your R script. Alternatively, highlight all your code together and run it either with the "Run" button or by using the keyboard shortcut (ctrl + enter on Windows, command + enter on Mac).